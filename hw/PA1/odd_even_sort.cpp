...
void merge(float *arr_a, int len_a, float *arr_b, int len_b, float *merged, bool is_left) {
// #ifdef DEBUG
//     printf("\n-------------------------------------\n");
...
// #endif // DEBUG
  ...
  if (is_left) {
    ...
    for (; r > l; r--) {
      if (p < 0) merged[r - len_a] = arr_b[q--];
      else if (q < 0) merged[r - len_a] = arr_a[p--];
      else merged[r - len_a] = arr_a[p] < arr_b[q] ? arr_b[q--] : arr_a[p--];
    }
  }
}
...
void Worker::sort() {
  ...
  while(iter < nprocs) {
    ...
    merge(data, block_len, recvData, recvSize, merged, master_rank);
    float *tmp = data;
